...
for r in range(nbre_repet):
    ...
    # begin: mrank, end: taille_nuage, step: nranks
    my_nuage = nuage[mrank:taille_nuage:nranks]
    t2 = time.time()
    elapsed_generation += t2 - t1

    # Calcul de l'enveloppe convexe :
    t1 = time.time()
    my_enveloppe = calcul_enveloppe(my_nuage)
    i = 0
    while (mrank % (2**i) == 0 and 2**i < nranks):
        crank = mrank // (2**i)
        if (crank % 2 == 0):
            source = mrank + 2**i
            env_source = comm.recv(source=source)
            my_enveloppe = np.append(my_enveloppe, env_source, axis=0)
            my_enveloppe = np.unique(my_enveloppe, axis=0)
            my_enveloppe = calcul_enveloppe(my_enveloppe)
        else:
            dest = mrank - 2**i
            comm.send(my_enveloppe, dest=dest)
        i += 1
    enveloppe = my_enveloppe
...
if (taille_nuage == 55440 and mrank == 0):
